...
namespace OpenTK_Samples
{
	...
	abstract class Model
	{
		...
		static public Color4 RGBAfromHSVA(float H, float S, float V, float A)
		{
		...
		public override void Initialize()
		{
			...
			for (int i = 0; i <= row; i++)
			{
				...
				for (int j = 0; j <= column; j++)
				{
					...
					Color4 color = Model.RGBAfromHSVA(360f * (float)j / (float)column, 1.0f, 1.0f, 1.0f);
		...
		public override void Initialize()
		{
			...
			for (int i = 0; i <= row; i++)
			{
				for (int j = 0; j <= column; j++)
				{
					...
					float z = (2.0f * (float)j / (float)(column + 1) - 1.0f) * size;

					Vector3 position = new Vector3(x, height, z);
					Vector3 normal = Vector3.UnitY;
					Color4 color = Color4.Gray;
