...
namespace OpenTK_Samples
{
	...
	class Game : GameWindow
	{
		...
		//800x600のウィンドウを作る。タイトルは「2-7:Mipmap」
		public Game()
			: base(800, 600, GraphicsMode.Default, "2-7:Mipmap")
		{
		...
		protected override void OnUpdateFrame(FrameEventArgs e)
		{
			...
			//Zキーで、縮小時最近傍補間 Mipmap最近傍補間
			if (Keyboard[Key.Z])
			{
				GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.NearestMipmapNearest);
			}

			//Xキーで、縮小時最近傍補間 Mipmap線形補間
			if (Keyboard[Key.X])
			{
				GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.NearestMipmapLinear);
			}

			//Cキーで、縮小時線形補間 Mipmap最近傍補間
			if (Keyboard[Key.C])
			{
				GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.LinearMipmapNearest);
			}

			//Vキーで、縮小時線形補間 Mipmap線形補間
			if (Keyboard[Key.V])
			{
				GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.LinearMipmapLinear);
			}

			//Bキーで、縮小時最近傍補間 Mipmap無し
			if (Keyboard[Key.B])
			{
