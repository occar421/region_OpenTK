...
namespace OpenTK_Samples
{
	...
	class Game : GameWindow
	{
		...
		//800x600のウィンドウを作る。タイトルは「2-9:Texture Replace」
		public Game()
			: base(800, 600, GraphicsMode.Default, "2-9:Texture Replace")
		{
		...
		protected override void OnLoad(EventArgs e)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				Bitmap file = new Bitmap("02-09_picture[" + i + "].png");
		...
		protected override void OnUpdateFrame(FrameEventArgs e)
		{
			...
			//Zキーで、描画したものをテクスチャとして現在のユニットへ流し込む
			if (Keyboard[Key.Z])
			{
			...
			//Cキーで、描画したものの一部を現在のユニットのテクスチャの一部とする
			if (Keyboard[Key.C])
			{
			...
			//Vキーで、プログラムの初期テクスチャに戻す
			if (Keyboard[Key.V])
			{
				GL.TexSubImage2D(TextureTarget.Texture2D, 0, 0, 0, textureWidth, textureHeight, OpenTK.Graphics.OpenGL.PixelFormat.Bgra, PixelType.UnsignedByte, textureData[0]);
				GL.ClearColor(Color4.Gray);
			}
